...
class Storage extends _nxusCore.NxusModule {
    ...
    for (var key in this.config.adapters) {
      if (_underscore2.default.isString(this.config.adapters[key])) {
        try {
          ...
          this.log.error('_setupAdapter config for', key, 'adapter:', this.config.adapters[key], 'not found in installed dependencies.');
        }
      }
    }
  }
  ...
  _disconnectDb() {
    var adapterNames = _underscore2.default.values(this._adapters).map(e => e['_name']);
    return _bluebird2.default.all(_underscore2.default.values(this._adapters), adapter => {
      ...
      return new _bluebird2.default(resolve => {
        ...
        let adaptersExp = new RegExp("^.*(" + adapterNames.join("|") + ")\\" + _path2.default.sep + ".*.js");
