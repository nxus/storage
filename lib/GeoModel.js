...
var _geojsonRewind = require('geojson-rewind');

var _geojsonRewind2 = _interopRequireDefault(_geojsonRewind);
...
_meta2.default.centroid = _centroid2.default;
...
function cleanPolygon(coordinates) {
  ...
  function same(c1, c2) {
    return near(c1[0], c2[0]) && near(c1[1], c2[1]);
  }
...
function cleanGeometryCoordinates(parts) {
  ...
  parts.Polygon.forEach(coordinates => {
    ...
    if (coordinates) {
      let obj = (0, _geojsonRewind2.default)({ type: 'Polygon', coordinates });
...
function extractGeometryCoordinates(obj) {
  ...
  function save(type, coordinates) {
    if (coordinates) parts[type].push(coordinates);
  }
  ...
  function extract(node) {
    switch (node.type) {
      case 'Polygon':
        ...
        break;
      case 'MultiPolygon':
        if (node.coordinates) node.coordinates.forEach(coordinates => {
      ...
      case 'MultiLineString':
        if (node.coordinates) node.coordinates.forEach(coordinates => {
          save('LineString', coordinates);
        });
        break;
      case 'GeometryCollection':
        if (node.geometries) node.geometries.forEach(geometry => {
          extract(geometry);
        });
        break;
    }
  ...
  if (obj) _meta2.default.geomEach(obj, extract);
  return parts;
}
...
function assembleGeometryObject(parts) {
  ...
  _underscore2.default.each(parts, (coordinates, type) => {
    if (coordinates.length > 0) {
      geometries.push(coordinates.length > 1 ? { type: 'Multi' + type, coordinates: coordinates } : { type: type, coordinates: coordinates[0] });
    }
  });

  return geometries.length === 0 ? undefined : geometries.length > 1 ? { type: 'GeometryCollection', geometries: geometries } : geometries[0];
}

function extractGeometryFeatures(values, next) {
  let val = values[this.geometryField],
      geo;
  if (val !== undefined) {
    try {
      if (_underscore2.default.isString(val)) val = JSON.parse(val);
 ...
 * type `json`.
 */
const GeoModel = _BaseModel2.default.extend(
/** @lends GeoModel */
{
  ...
  getGeometry: function (record, ...types) {
    let val = record[this.geometryFeatureField],
        geo;
    if (val) {
      if (types.length === 0) types.push('Polygon');
      try {
        let parts = extractGeometryCoordinates(val);
        geo = assembleGeometryObject(_underscore2.default.pick(parts, types));
      } catch (e) {}
    }
    return geo;
  },
  ...
  getCentroid: function (record) {
    ...
    if (val) {
      try {
        let feature = _meta2.default.centroid(val);
    ...
    return new _bluebird2.default((resolve, reject) => {
      this.native((err, collection) => {
        ...
        collection.find(geoQuery, { _id: 1 }).toArray((err, objs) => {
          ...
          let wlQuery = { 'id': _underscore2.default.pluck(objs, '_id') };
          resolve(() => {
            return this.find(wlQuery);
          });
        });
      });
    });
